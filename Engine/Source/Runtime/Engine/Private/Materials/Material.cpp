...
void UMaterial::PostLoad()
{
	...
	//NE-START
	DoMaterialAttributeReorder(&CustomExColor, UE4Ver, RenderObjVer);
	//NE-END
#endif // WITH_EDITORONLY_DATA
...
FExpressionInput* UMaterial::GetExpressionInputForProperty(EMaterialProperty InProperty)
{
	switch (InProperty)
	{
		...
		//NE-START
		case MP_CustomData2Float3:		return &CustomExColor;
		//NE-END
...
int32 UMaterial::CompilePropertyEx( FMaterialCompiler* Compiler, const FGuid& AttributeID )
{
	...
	switch (Property)
	{
		...
		//NE-START
		case MP_CustomData2Float3:		return CustomExColor.CompileWithDefault(Compiler, Property);
		//NE-END
	...
	bool bUsesShadingModelFromMaterialExpression)
{
	...
	case MP_SubsurfaceColor:
		Active = ShadingModels.HasAnyShadingModel({ MSM_Subsurface, MSM_PreintegratedSkin, MSM_TwoSidedFoliage, MSM_Cloth, MSM_ApproximatingTranslucency, MSM_PreintegratedCheapSSS});
		break;
	case MP_CustomData0:
		Active = ShadingModels.HasAnyShadingModel({ MSM_ClearCoat, MSM_Hair, MSM_Cloth, MSM_Eye, MSM_ApproximatingTranslucency});
		break;
	case MP_CustomData1:
		Active = ShadingModels.HasAnyShadingModel({ MSM_ClearCoat, MSM_Eye, MSM_ApproximatingTranslucency});
		break;
	case MP_CustomData2Float3:
		Active = ShadingModels.HasAnyShadingModel({ MSM_ApproximatingTranslucency, MSM_PreintegratedCheapSSS });
		break;
	//NE-END
