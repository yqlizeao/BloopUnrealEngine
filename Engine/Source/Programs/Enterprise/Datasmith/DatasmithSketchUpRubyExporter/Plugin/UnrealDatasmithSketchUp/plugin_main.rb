...
module EpicGames
  module Loc
    ...
    def self.load(locale_name)
      if $epicgames_datasmith_sketchup_resources
        localization_fpath = File.join($epicgames_datasmith_sketchup_resources, locale_name, 'plugin.strings')
  ...
  module Datasmith
    Datasmith.log_init $epicgames_datasmith_sketchup_dev
    ...
    # set active path to top and revert back
    ...
    def self._load(dll_path)
      Datasmith.log_info { "Loading '#{__FILE__}'..." }

      major, minor, build = Sketchup.version.split('.').map {|s| s.to_i}

      @ruby_api_supported = (major >= 20) && (minor >=2)
      @active_path_change_supported = major >= 20
      @enable_directlink = $epicgames_datasmith_sketchup_directlink_enabled # disable DirectLink for SU earlier than 2020

      @plugin_path = __FILE__

      # Load extension
      @module_path = dll_path
      Datasmith.log_info  { "Loading plugin binary module from #{@module_path}"}
      require @module_path
      ...
      resources_path = $epicgames_datasmith_sketchup_resources
      ...
      send_update_command.tooltip = Loc.t :SynchronizeTooltip
      ...
      auto_sync_toggle_command.tooltip = Loc.t :ToggleAutoSyncTooltip
      auto_sync_toggle_command.small_icon = File.join(resources_path, 'DatasmithAutoSyncIcon16.png')
      auto_sync_toggle_command.large_icon = File.join(resources_path, 'DatasmithAutoSyncIcon32.png')
      ...
      manage_connections_command.tooltip = Loc.t :ManageConnectionsTooltip
      ...
      export_command.tooltip = Loc.t :ExportModelToDatasmith
      ...
      show_log_command.tooltip = Loc.t :ShowLogTooltip
      ...
      toolbar.add_item send_update_command if @enable_directlink
      toolbar.add_item auto_sync_toggle_command if @enable_directlink
      toolbar.add_item manage_connections_command if @enable_directlink
      toolbar.add_item export_command
      toolbar.add_item show_log_command if @enable_directlink

      extensions_menu = UI.menu'Extensions'
      datasmith_menu = extensions_menu.add_submenu Loc.t :Datasmith
      datasmith_menu.add_item send_update_command if @enable_directlink
      datasmith_menu.add_item auto_sync_toggle_command if @enable_directlink
      datasmith_menu.add_item manage_connections_command if @enable_directlink
      datasmith_menu.add_item export_command
      datasmith_menu.add_item show_log_command if @enable_directlink

      if $epicgames_datasmith_sketchup_dev
      ...
      if live_update?
        ...
        @live_update_timer = UI.start_timer 1, true do
          ...
          if allowed_tools.include? Sketchup.active_model.tools.active_tool_name
            if @scene_modified || !@change_tracking_started
              send_update
    ...
    def self.init_directlink
      ...
      if Sketchup.platform == :platform_win
        if $epicgames_datasmith_sketchup_engine_path.nil?
          engine_path_for_slate = get_platform_engine_path
        else
          # for dev
          engine_path_for_slate = $epicgames_datasmith_sketchup_engine_path
        end
      end
    ...
    def self.export_current_datasmith_scene
      ...
      # to relative - when called on first export all is file(source path is absolute)
    ...
    def self.send_update
      ...
      return unless @enable_directlink
    ...
    def self.ensure_directlink_activated_for_model
      ...
      @exporter = EpicGames::DatasmithBackend::DatasmithSketchUpDirectLinkExporter.new(@name, @export_folder, @enable_directlink)
      @change_tracking_started = false
    end

    def self.update_directlink_scene
      Datasmith.log_info { "update_scene" }
    ...
    module ObserverHelperDecoratorImpl
      ...
      def self.included(base)
        EpicGames::Datasmith::ObserverHelperDecorator.install(self, base, !$epicgames_datasmith_sketchup_dev)
      end
    ...
    class EntitiesObserver < Sketchup::EntitiesObserver
      ...
      def onElementRemoved(entities, entity_id)
        ...
        result = Datasmith.timeit('on_entity_removed') {
          EpicGames::Datasmith.get_exporter.on_entity_removed(EpicGames::Datasmith.get_entity_id(entities.parent), entity_id)
        }
    ...
    class RenderingOptionsObserver < Sketchup::RenderingOptionsObserver
      ...
      def onRenderingOptionsChanged(rendering_options, type)
        ...
        if type == Sketchup::RenderingOptions::ROPSetFaceColor
    ...
    unless file_loaded?(__FILE__)
      ...
      if load $epicgames_datasmith_sketchup_dll
